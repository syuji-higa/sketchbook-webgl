...
// import shader
const mainOffscreenVertexShader = require('./shaders/main-offscreen.vert')
const mainOffscreenFragmentShader = require('./shaders/main-offscreen.frag')
...
class MainObject {
  ...
  constructor(dpr: number /* [0,inf) */) {
    ...
    if (tweakpane) {
      ...
      _f.addInput(this._params, 'power', { min: 0, max: 0.99 })
    }
  }
  ...
  draw(time: number /* int[0,inf) */, mat: Mat, size: Size): MainObject {
    ...
    // before texture (offscreen rendering)
    {
      // create frame buffer
      const { frameBuffer, frameTexture } = createFramebuffer(
        this._gl,
        size.w,
        size.h
      )
      ...
      const { prg, attLocs, attStrides, uniLocs } = programData.offscreen
      ...
      this._matIV.scale(mat.m, [size.wR, 1, 0], mat.m) // contain
      this._matIV.multiply(mat.tmp, mat.m, mat.mvp)
      ...
      this._gl.bindTexture(this._gl.TEXTURE_2D, this._texture || texture)
      this._gl.uniform1i(uniLocs['beforeTexture'], 0)
      this._gl.activeTexture(this._gl.TEXTURE1)
      this._gl.bindTexture(this._gl.TEXTURE_2D, texture)
      this._gl.uniform1i(uniLocs['currentTexture'], 1)
  ...
  private _createMainObject(): MainObjectData {
    ...
    const _uniLocsOffscreen: string[] = []
    ...
    // set uniform offscreen location
    _uniLocsOffscreen.push(
      'mvpMatrix',
      'time',
      'resolution',
      'beforeTexture',
    ...
    _uniLocsDefault.push('mvpMatrix', 'time', 'resolution', 'texture')

    // offscreen program
    _programData.offscreen = createProgramData(
      this._gl,
      _attLocs,
      _uniLocsOffscreen,
      _attStrides,
      mainOffscreenVertexShader,
      mainOffscreenFragmentShader
    )
