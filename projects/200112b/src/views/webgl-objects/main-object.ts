...
class MainObject {
  ...
  async _update() {
    ...
    if (0 < width && 0 < height) {
      ...
      if (_segmentation.allPoses.length) {
        const { keypoints } = _segmentation.allPoses[0]

        {
          const { x: leftX, y: leftY } = keypoints[9].position
          const { x: rightX, y: rightY } = keypoints[10].position
          this._poseState.hands[0] = (leftX * 2 - width) / width
          this._poseState.hands[1] = (leftY * 2 - height) / height
          this._poseState.hands[2] = (rightX * 2 - width) / width
          this._poseState.hands[3] = (rightY * 2 - height) / height
        }

        {
          const { x: leftX, y: leftY } = keypoints[7].position
          const { x: rightX, y: rightY } = keypoints[8].position
          this._poseState.elbows[0] = (leftX * 2 - width) / width
          this._poseState.elbows[1] = (leftY * 2 - height) / height
          this._poseState.elbows[2] = (rightX * 2 - width) / width
          this._poseState.elbows[3] = (rightY * 2 - height) / height
        }
      }
