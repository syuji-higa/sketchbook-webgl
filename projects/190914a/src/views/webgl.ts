...
class WebGL {
  ...
  create(): WebGL {
    ...
    this._matrix = {
      ...
      inv: this._matIV.identity(this._matIV.create()) // mvp inverse
    }
  ...
  async loadModelObject(
    id: string,
    uri: string,
    path: string
  ): Promise<boolean> {
    const _object = this._objects[id]
    const _isSuccess = await _object.loadModel(uri, path)
    return _isSuccess
  }

  createObject(id: string): WebGL {
    const _object = this._objects[id]
